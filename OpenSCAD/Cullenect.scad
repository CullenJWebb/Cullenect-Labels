...
// Select View Type
Fastener_View = "side"; // [side:Side, top:Top]
  ...
  difference() {
    ...
    union() {
      ...
      translate([0, vsocketY, 0])
        cube([vsocketX, latchZ, vsocketZ]);
      // Middle of socket, to be cut away later by rounded cube for rib
...
module cullenect_driver(
  ...
  // Torx
  module torx() {
    module torx_cyl() { cylinder(h=layer, d=driverWidth, center=true, $fa=1); }; module torx_long(length = driverLength - driverWidth, cube = true) {
      // Connecting cube
...
module cullenect_head(
  ...
  // Top/Drive View
  module top_view() {
    cylinder(h=layer, d=headY, center=true);
  }
...
module cullenect_shaft(shaft = "machine", threads = "full") {
  ...
  module threads(num = thread_num, pos = thread_pos_x) {
    translate([-pos, 0, 0])for (i = [0:1:num - 1]) {
    ...
    translate([-hardX / 2, -hardX / 2, -layer / 2])
      difference() {
        union() {
          ...
          translate([1, 0, 0])
            cube([hardX - 2, hardX, layer]);
          // Middle of insert
          translate([0, hardX * 0.75, 0])
            RoundedCube([hardX, hardX / 4, layer], 0.2);
          // Bottom of insert
        }
        ...
        translate([hardX - hardX / 5, hardX / 4, 0])
          RoundedCube([hardX / 5 + 1, hardX / 2, layer], 0.2);
        translate([hardX * 0.06, hardX * 0.19, 0])for (i = [0:1:4]) {
    ...
    translate(fastener_pos) {
      union() {
        if (fastenerView == "top") {
          translate([-3, 0, 0]) {
            difference() {
              cullenect_head(head=fastenerHead, flange=fastenerHeadFlange, view="top");
          ...
          difference() {
            cullenect_head(head=fastenerHead, flange=fastenerHeadFlange, view="side");
            cullenect_driver(driver=fastenerDriver);
          }
...
module selected_model() {
  if (Select_Output == 10) {
    cullenect_socket();
  } else if (Select_Output == 11) {
    cullenect_socket_negative();
  } else if (Select_Output == 20) {
    cullenect_vertical_socket();
  } else if (Select_Output == 21) {
    cullenect_vertical_socket_negative();
  } else {
